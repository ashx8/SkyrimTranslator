...
namespace SkyrimModTranslator.UI
{
    ...
    public partial class Dictw : Window
    {
        ...
        private ItemList _otherList;
        private OptionButton _modSelector;
        private OptionButton _otherSel;
        ...
        private Dictionary<string, Dictionary<string, string>> _otherDicts = new();
        private Dictionary<Button, string> _buttonToShard = new Dictionary<Button, string>();
        private string _currMod = "";
        private string _curOtherDict = "";
        private string _editOriKey;
        private VBoxContainer _otherShardPanel;
        private string _curOtherShard = "a";
        private Dictionary<string, string> _otherDictData = new();
        ...
        private async void RefreshShardButtons()
        {
            if (_tabContainer.CurrentTab == 2)
            {
                foreach (var child in _otherShardPanel.GetChildren()) child.QueueFree();
            }
            else
            {
            ...
            string category;
            string sub;
            VBoxContainer targetPanel;
            
            if (_tabContainer.CurrentTab == 0)
            {
                category = "UserDict";
                sub = "";
                targetPanel = _shardPanel;
            }
            ...
            string dirPath = SkyrimModTranslator.Core.Dict.DictStorage.GetTargetDir(category, sub);
            ...
            foreach (var fileName in files)
            {
                ...
                targetPanel.AddChild(btn);
            }

            if (files.Count > 0)
            {
                string firstShard = files[0];
                if (_tabContainer.CurrentTab == 2)
                {
                    _curOtherShard = firstShard;
                    await RefreshOtherList();
                }
                else
                {
                    _currShard = firstShard;
                    await RefreshGlossaryList();
                }
                UpdateShardPanelStyles();
            }
        }

        private async void OnShardButtonPressed(string shardName)
        {
            if (_tabContainer.CurrentTab == 2)
            {
                _curOtherShard = shardName;
                await RefreshOtherList();
            }
            else
            {
        ...
        private void SetupUI()
		{
			...
			var otherShardScroll = new ScrollContainer();
			otherShardScroll.CustomMinimumSize = new Vector2(45, 0);
			otherShardScroll.HorizontalScrollMode = ScrollContainer.ScrollMode.Disabled;
			otherShardScroll.Visible = false;
			contentHBox.AddChild(otherShardScroll);

			var otherShardPanel = new VBoxContainer();
			otherShardPanel.SizeFlagsHorizontal = Control.SizeFlags.ExpandFill;
			otherShardScroll.AddChild(otherShardPanel);
            ...
            otherBox.AddThemeConstantOverride("separation", 10);

            //下拉框让用户选其他词典
            var otherHBox = new HBoxContainer();
            otherHBox.AddThemeConstantOverride("separation", 10);
            var otherLabel = new Label { Text = L.T("LBL_OTHER_DICT"), SizeFlagsHorizontal = Control.SizeFlags.ShrinkEnd };
            otherHBox.AddChild(otherLabel);
            _otherSel = new OptionButton { SizeFlagsHorizontal = Control.SizeFlags.ExpandFill };
            _otherSel.ItemSelected += OnOtherDictSelected;
            otherHBox.AddChild(_otherSel);
            _btnSetDefault = new Button { Text = L.T("BTN_SET_DEFAULT"), CustomMinimumSize = new Vector2(100, 35) };
            _btnSetDefault.Pressed += OnSetDefaultPressed;
            otherHBox.AddChild(_btnSetDefault);
            otherBox.AddChild(otherHBox);

            _otherList = CreateGlossaryList("OtherDict", otherBox);
            _tabContainer.AddChild(otherBox);
            _tabContainer.SetTabTitle(_tabContainer.GetChildCount() - 1, L.T("TAB_OTHER"));

            //把控件引用存起来，后面要用
            _otherShardPanel = otherShardPanel;
        ...
        private void OnEditEntry(ItemList list, int idx)
        {
            ...
            else if (list == _otherList)
                _otherDictData.TryGetValue(_editOriKey, out translation);
        ...
        private async void OnTextSaved(string newTrans, string newOri)
        {
            ...
            if (_editOriKey != newOri) {
                if (currentTab == 0) {
                    ...
                    _otherDictData.Remove(_editOriKey);
                    SkyrimModTranslator.Core.Dict.DictStorage.Delete("OtherDict", _curOtherDict, _editOriKey);
                }
            }
            ...
            if (currentTab == 0) {
                ...
                _otherDictData[newOri] = newTrans;
                SkyrimModTranslator.Core.Dict.DictStorage.Save("OtherDict", _curOtherDict, updateDict);
            }
        ...
        private async void OnEditorTextSaved(string newTrans, string newOri, TransEntry entry, bool isNew)
        {
            int currentTab = _tabContainer.CurrentTab;
            if (currentTab == 0)
            {
            ...
            else if (currentTab == 2 && !string.IsNullOrEmpty(_curOtherDict))
            {
                if (!isNew) _otherDictData.Remove(entry.Ori);
                _otherDictData[newOri] = newTrans;
                var updateDict = new Dictionary<string, string> { { newOri, newTrans } };
                SkyrimModTranslator.Core.Dict.DictStorage.Save("OtherDict", _curOtherDict, updateDict);
            }
        ...
        private void OnOpenFolderPressed()
        {
            ...
            if (currentTab == 0)
                ...
                targetPath = Path.Combine(langPath, "ModDict");
            else if (currentTab == 2)
                targetPath = Path.Combine(langPath, "OtherDict");
        ...
        private async void OnTabChanged(long tabIndex)
        {
            foreach (var child in _shardPanel.GetChildren()) child.QueueFree();
            foreach (var child in _otherShardPanel.GetChildren()) child.QueueFree();
            ...
            if (tabIndex == 2)
            {
                _otherShardPanel.GetParent<ScrollContainer>().Visible = true;
                _shardPanel.GetParent<ScrollContainer>().Visible = false;
            }
            else
            {
                _shardPanel.GetParent<ScrollContainer>().Visible = true;
                _otherShardPanel.GetParent<ScrollContainer>().Visible = false;
            }
        ...
        private void OnAddRequest()
        {
            ...
            if (currentTab == 2 && string.IsNullOrEmpty(_curOtherDict))
            {
                UpdateStat("请先选择一个其他词典");
                return;
            }
        ...
        private async System.Threading.Tasks.Task RefreshGlossaryList() 
        {
            ...
            if (string.IsNullOrEmpty(searchText))
            {
                ...
                UpdateStat(L.T("STAT_SHARD_ENTRIES").Replace("{shard}", _currShard.ToUpper()).Replace("{count}", data.Count.ToString()));
            }
            else
            {
                ...
                UpdateStat(L.T("STAT_GLOBAL_SEARCHING").Replace("{search}", searchText));
            }
            ...
            if (!string.IsNullOrEmpty(searchText))
            {
                UpdateStat(L.T("STAT_SEARCH_RESULTS").Replace("{count}", targetList.ItemCount.ToString()));
            }
        ...
        private async void OnSearchChanged(string text)
        {
            ...
            if (_tabContainer == null || !IsInstanceValid(_tabContainer))
            {
                return;
            }
        ...
        private void OnDeletePressed()
        {
            ...
            {
                list = _otherList;
                currentGlossary = _otherDictData;
                category = "OtherDict";
                modName = _curOtherDict;
            }
            ...
            confirmDialog.GetOkButton().Text = L.T("BTN_YES");
            confirmDialog.GetCancelButton().Text = L.T("BTN_NO");
        ...
        private void OnMajorImportPressed()
        {
            ...
            dialog.Title = L.T("TITLE_IMPORT_TARGET");
            dialog.DialogText = L.T("MSG_SELECT_IMPORT_TYPE");
            ...
            dialog.AddButton(L.T("BTN_OTHER_DICT"), true, "other");
            ...
            dialog.CustomAction += (action) => {
                ...
                else if (action.ToString() == "other") OpenOtherDictImport();
                dialog.QueueFree();
            };
        ...
        private void OpenOtherDictImport()
        {
            var win = new ImpDict();
            win.Done += () => {
                if (_tabContainer.CurrentTab == 2)
        ...
        private void LoadOtherDicts()
        {
            _otherSel.Clear();
            _otherDicts.Clear();
            
            string defaultDict = Pos.GetSetting("default_other_dict", "");
            var otherDicts = SkyrimModTranslator.Core.Dict.DictStorage.GetModList("OtherDict");
            
            if (!string.IsNullOrEmpty(defaultDict) && !otherDicts.Contains(defaultDict))
            {
                defaultDict = "";
                Pos.SaveSetting("default_other_dict", "");
            }
            
            if (otherDicts.Count == 1 && string.IsNullOrEmpty(defaultDict))
            {
                defaultDict = otherDicts[0];
                Pos.SaveSetting("default_other_dict", defaultDict);
            }
            
            if (otherDicts.Count > 0 && string.IsNullOrEmpty(defaultDict))
            {
                defaultDict = otherDicts[0];
                Pos.SaveSetting("default_other_dict", defaultDict);
            }
            ...
            {
                string displayName = dict;
                if (dict == defaultDict)
                {
                    displayName = $"[默认] {dict}";
                }
                _otherSel.AddItem(displayName);
            }
            
            if (_otherSel.ItemCount > 0)
            {
                int selectIndex = 0;
                if (!string.IsNullOrEmpty(defaultDict))
                {
                    for (int i = 0; i < otherDicts.Count; i++)
                    {
                        if (otherDicts[i] == defaultDict)
                        {
                            selectIndex = i;
                            break;
                        }
                ...
                _otherSel.Select(selectIndex);
                OnOtherDictSelected(selectIndex);
            }
            else
            {
                _otherList.Clear();
                foreach (var child in _otherShardPanel.GetChildren())
                    child.QueueFree();
            }
        }
        
        private void OnSetDefaultPressed()
        {
            if (string.IsNullOrEmpty(_curOtherDict)) return;
            
            string defaultDict = Pos.GetSetting("default_other_dict", "");
            string selectedDict = _curOtherDict;
            
            if (selectedDict.StartsWith("[默认] "))
            {
                selectedDict = selectedDict.Substring(5);
            }
            
            if (defaultDict == selectedDict)
            {
                Pos.SaveSetting("default_other_dict", "");
            }
            else
            {
                Pos.SaveSetting("default_other_dict", selectedDict);
            }
        ...
        private async void OnOtherDictSelected(long idx)
        {
            string displayName = _otherSel.GetItemText((int)idx);
            _curOtherDict = displayName.StartsWith("[默认] ") ? displayName.Substring(5) : displayName;
            _otherDictData.Clear();
            _otherDictData = SkyrimModTranslator.Core.Dict.DictStorage.Load("OtherDict", _curOtherDict);
            RefreshShardButtons();
            
            var shardButtons = _otherShardPanel.GetChildren();
            if (shardButtons.Count > 0)
            {
                foreach (var child in shardButtons)
                {
        ...
        private async System.Threading.Tasks.Task RefreshOtherList() 
        {
            if (_isLoading) return;
            _isLoading = true;

            string searchText = _searchBar.Text.ToLower().Trim();
            if (string.IsNullOrEmpty(_curOtherDict))
            {
                _isLoading = false;
                return;
            }
            ...
            if (string.IsNullOrEmpty(searchText))
            {
                data = SkyrimModTranslator.Core.Dict.DictStorage.LoadByShard("OtherDict", _curOtherDict, _curOtherShard);
                UpdateStat(L.T("STAT_SHARD_ENTRIES").Replace("{shard}", _curOtherShard.ToUpper()).Replace("{count}", data.Count.ToString()));
            }
            else
            {
                data = await System.Threading.Tasks.Task.Run(() => 
                    SkyrimModTranslator.Core.Dict.DictStorage.Load("OtherDict", _curOtherDict, null)
                );
                UpdateStat(L.T("STAT_GLOBAL_SEARCHING").Replace("{search}", searchText));
            }
            ...
            foreach (var kvp in data)
            {
                if (string.IsNullOrEmpty(searchText) || 
                    ...
                    kvp.Value.ToLower().Contains(searchText))
                {
                    _otherList.AddItem($"{kvp.Key} -> {kvp.Value}");
                    _otherList.SetItemMetadata(_otherList.ItemCount - 1, kvp.Key);
            ...
            if (!string.IsNullOrEmpty(searchText))
            {
                UpdateStat(L.T("STAT_SEARCH_RESULTS").Replace("{count}", _otherList.ItemCount.ToString()));
            }
        ...
        private async void OnDeleteConfirmed(int[] selected, Dictionary<string, string> currentGlossary, string category, string modName)
        {
            ...
            if (category == "UserDict" || category == "ModDict")
            {
                SaveGlossary();
            }
        ...
        private void OnDeleteAllUserPressed()
        {
            ...
            else if (currentTab == 2)
            {
                ...
                msg = L.T("MSG_CONFIRM_DELETE_MOD_ALL").Replace("{mod}", _curOtherDict); 
                deleteAction = () => {
                    SkyrimModTranslator.Core.Dict.DictStorage.ClearAll("OtherDict", _curOtherDict, true);
                    _otherDictData.Clear();
            ...
            dialog.GetOkButton().Text = L.T("BTN_YES");
            dialog.GetCancelButton().Text = L.T("BTN_NO");
        ...
        private void ExportGlossary(string path)
        {
            ...
            {
                int currentTab = _tabContainer.CurrentTab;
                Dictionary<string, string> exportDict;
                
                if (currentTab == 0)
                {
                    exportDict = _userGlossary;
                }
                else if (currentTab == 1)
                {
                    exportDict = _modGlossary;
                }
                else if (currentTab == 2)
                {
                    exportDict = _otherDictData;
                }
                else
                {
        ...
        private void UpdateStat(string message)
        {
            ...
            {
                var dict = SkyrimModTranslator.Core.Dict.DictStorage.Load("OtherDict", _curOtherDict);
                _lblStat.Text = L.T("STATUS_DICT_ENTRIES").Replace("{num}", dict.Count.ToString());
            }
            else
            {
        ...
        public override void _Input(InputEvent @event)
        {
            if (@event is InputEventKey keyEvent && keyEvent.Pressed)
            {
                ...
                {
                    var currentList = _tabContainer.GetChild(_tabContainer.CurrentTab) as ItemList;
                    if (currentList != null)
                    {
        ...
        private void OnEditExportPromptPressed()
        {
            ...
            var btnRestoreDefault = new Button { Text = L.T("BTN_RESTORE_DEFAULT"), CustomMinimumSize = new Vector2(100, 40) };
            ...
            btnRestoreDefault.Pressed += () => {
                string defaultPrompt = L.T("PROMPT_PREFIX_GLOSSARY") + L.T("PROMPT_BASE");
                textEdit.Text = defaultPrompt;
            };
            ...
            hBox.AddChild(btnRestoreDefault);
