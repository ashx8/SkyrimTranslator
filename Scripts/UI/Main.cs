...
namespace SkyrimModTranslator.UI
{
    ...
    public partial class Main : Control  {
        ...
        private Dictionary<string, List<Data.Item>> _pendingItems = new Dictionary<string, List<Data.Item>>();
        private Dictionary<string, bool> _tabPopulated = new Dictionary<string, bool>();
        private Dictionary<string, bool> _tabSearchMode = new Dictionary<string, bool>();
        private Dictionary<string, string> _tabSearchText = new Dictionary<string, string>();
        private Dictionary<string, int> _columnSort = new Dictionary<string, int>();
        ...
        private Dictionary<string, string> _cachedDictionary = null;
        private string _lastModPath = null;
        private DateTime _lastDictionaryAccessTime = DateTime.MinValue;
        private bool _populateRunning = false;
        private int _populateSearchId = 0;
        private bool _isApplyingDictionary = false;
        private SceneTreeTimer _watchdogTimer;
        private Stopwatch _mainThreadWatchdog;
        ...
        public override void _Ready()
        {
            ...
            _parser = new Parser();
            _parser.TranslationMap = Cfg.LoadTransMap();
            ...
            //先把字符串加载器准备好，后面解析要用
            var strLoader = new StrLoader();
            _parser.SetStrLoader(strLoader);
            strLoader.LoadAll();
        ...
        private void CreateLayout()
        {
            ...
            _loadingTip.AddThemeFontSizeOverride("font_size", 24);
            _loadingTip.AddThemeColorOverride("font_color", new Color(1, 0.8f, 0.2f));
            _loadingTip.SetAnchorsPreset(Control.LayoutPreset.Center);
            _loadingTip.OffsetLeft = -300;
            _loadingTip.OffsetRight = 300;
            _loadingTip.OffsetTop = 60;
            _loadingTip.OffsetBottom = 110;
            AddChild(_loadingTip);
            ...
            _btnGSearch = new Button { Text = L.T("BTN_G_SEARCH"), CustomMinimumSize = new Vector2(100, 40) };
            _btnGSearch.Pressed += OnGSearchPressed;
            titleBar.AddChild(_btnGSearch);
        ...
        private async void OnFilesDropped(string[] files)
        {
            ...
            foreach (var path in files)
            {
                if (Directory.Exists(path))
                {
                    ...
                    {
                        GD.PrintErr($"[Main] 扫描目录失败: {path}, 错误: {ex.Message}");
                    }
                }
                else if (File.Exists(path))
                {
                    if (path.EndsWith(".esp", StringComparison.OrdinalIgnoreCase) || 
            ...
            {
                await LoadModsBatchAsync(modsToLoad);
            }
            else
            {
                RefreshStatusBar();
        ...
        private async System.Threading.Tasks.Task LoadModsBatchAsync(List<string> modsToLoad)
        {
            ...
            {
                Title = L.T("MOD_LOADING_TITLE"),
                MinSize = new Vector2I(400, 100),
                Size = new Vector2I(500, 120),
                Transient = true,
                Exclusive = true
            };
            ...
            progressDialog.AddChild(margin);

            var vBox = new VBoxContainer();
            vBox.AddThemeConstantOverride("separation", 15);
            margin.AddChild(vBox);

            var progressLabel = new Label { Text = L.T("MOD_LOADING_PREPARE") };
            vBox.AddChild(progressLabel);

            var progressBar = new ProgressBar
            {
                MinValue = 0,
                MaxValue = modsToLoad.Count,
                Value = 0,
                SizeFlagsHorizontal = Control.SizeFlags.ExpandFill
            };
            vBox.AddChild(progressBar);

            GetTree().Root.AddChild(progressDialog);
            progressDialog.PopupCentered();

            try
            {
                int batchSize = 100;
                for (int i = 0; i < modsToLoad.Count; i += batchSize)
                {
                    int end = Math.Min(i + batchSize, modsToLoad.Count);
                    var batch = modsToLoad.GetRange(i, end - i);
                    
                    foreach (var modPath in batch)
                    {
                        try
                        {
                            await LoadMod(modPath);
                        }
                        catch (Exception ex)
                        {
                            GD.PrintErr($"[Main] 加载模组失败: {modPath}, 错误: {ex.Message}");
                        }
                        finally
                        {
                            int loaded = i + batch.IndexOf(modPath) + 1;
                            progressBar.Value = loaded;
                            progressLabel.Text = $"{L.T("MOD_LOADING_CURRENT")}: {Path.GetFileName(modPath)}\n{L.T("MOD_LOADING_PROGRESS")} {loaded}/{modsToLoad.Count}";
                ...
                progressLabel.Text = L.T("MOD_LOADING_COMPLETE");
                await System.Threading.Tasks.Task.Delay(1000);
            }
            catch (Exception ex)
            {
                GD.PrintErr($"[Main] 批量加载异常: {ex.Message}\n{ex.StackTrace}");
                progressLabel.Text = $"{L.T("MOD_LOADING_ERROR")}: {ex.Message}";
                await System.Threading.Tasks.Task.Delay(2000);
            }
            finally
            {
                progressDialog.QueueFree();
        ...
        private async System.Threading.Tasks.Task LoadMod(string path)
        {
            var totalSw = Stopwatch.StartNew();
            string modName = System.IO.Path.GetFileName(path);
            
            string modBase = System.IO.Path.GetFileNameWithoutExtension(path);
            
            var strSw = Stopwatch.StartNew();
            _parser.LoadStr(modBase);
            strSw.Stop();
            ...
            {
                GD.PrintErr($"[Main] 加载模组失败: {path}, 错误: {ex.Message}");
                RefreshStatusBar();
            }
        }
        ...
        public async System.Threading.Tasks.Task ReloadAllMods()
        {
            _parser.TranslationMap = Cfg.LoadTransMap();
        ...
        {
            if (_statusBar != null && progress >= 0 && progress <= 1 && _currentProject != null)
            {
                _statusBar.Update(System.IO.Path.GetFileName(_currentProject.Path), _currentProject.Items.Count, 0);
            }
        }

        private async void OnModSelected(long index)
        {
            if (index < 0) return;
            await SwitchProjectAsync((int)index);
        }

        private async void SwitchProject(int index)
        {
            await SwitchProjectAsync(index);
        }

        private async System.Threading.Tasks.Task SwitchProjectAsync(int index)
        {
            var sw = Stopwatch.StartNew();
            GD.Print($"[性能] 开始切换模组, 索引={index}, 模组={Path.GetFileName(_projects[index].Path)}");
            ...
            // 异步应用词典
            await ApplyDictionaryOnSwitchAsync(_currentProject);

            GD.Print($"[性能] 词典应用完成, 耗时={sw.ElapsedMilliseconds}ms");

            // 刷新UI（内部已改为异步过滤和分块填充）
            RefreshUI();

            RefreshStatusBar();
            // 注意: HideLoadingIndicator() 已移至 PopulateTreeChunked 末尾，
            // 因为 FilterAndPopulate 是 async void，这里无法等待其完成

            GD.Print($"[性能] 模组切换主流程完成, 耗时={sw.ElapsedMilliseconds}ms (UI填充仍在后台进行)");
        }
        ...
        private void ShowLoadingIndicator()
        {
            if (_loadingTip != null)
            {
                _loadingTip.Text = "正在加载，请稍候...";
                _loadingTip.Visible = true;
            }
            _mainThreadWatchdog = Stopwatch.StartNew();
        }
        ...
        private void HideLoadingIndicator()
        {
            ...
            if (_mainThreadWatchdog != null)
            {
                GD.Print($"[性能] 主线程阻塞检测: 最大持续={_mainThreadWatchdog.ElapsedMilliseconds}ms");
                _mainThreadWatchdog = null;
            }
        }
        ...
        {
            _mainThreadWatchdog = Stopwatch.StartNew();
            _watchdogTimer = GetTree().CreateTimer(2.0);
            _watchdogTimer.Timeout += OnWatchdogTimeout;
        }
        ...
        {
            if (_mainThreadWatchdog != null && _mainThreadWatchdog.IsRunning)
            {
                long elapsed = _mainThreadWatchdog.ElapsedMilliseconds;
                GD.PrintErr($"[卡死检测] 主线程持续阻塞 {elapsed}ms，当前状态: 词典应用={_isApplyingDictionary}, 填充中={_populateRunning}");
                if (_loadingTip != null && _loadingTip.Visible)
                {
                    _loadingTip.Text = $"加载中... (已耗时 {elapsed}ms)";
                }
                _watchdogTimer = GetTree().CreateTimer(2.0);
                _watchdogTimer.Timeout += OnWatchdogTimeout;
            }
        ...
        private void RefreshUI()
        {
            ...
            ClearTabs();
            clearSw.Stop();
            GD.Print($"[性能] ClearTabs 耗时={clearSw.ElapsedMilliseconds}ms");
            ...
            mapSw.Stop();
            GD.Print($"[性能] 构建_map 耗时={mapSw.ElapsedMilliseconds}ms, 条目数={_currentProject.Items.Count}");
            ...
            refreshSw.Stop();
            GD.Print($"[性能] RefreshUI 同步部分完成, 清Tabs={clearSw.ElapsedMilliseconds}ms, 建Map={mapSw.ElapsedMilliseconds}ms, 标签页={tabSw.ElapsedMilliseconds}ms, 同步总计={refreshSw.ElapsedMilliseconds}ms");
        }
        ...
        private async void FilterAndPopulate()
        {
            var filterSw = Stopwatch.StartNew();
            _populateSearchId++;
            int currentSearchId = _populateSearchId;
            ...
            int totalItems = _currentProject.Items.Count;

            GD.Print($"[性能] 开始过滤, 总词条={totalItems}, 搜索='{searchText}', 仅未翻译={filterUntranslated}");

            // 异步过滤并按分类分组
            var grouped = await System.Threading.Tasks.Task.Run(() =>
            {
                var groups = new Dictionary<string, List<Data.Item>>();
                foreach (Data.Item e in _currentProject.Items)
                {
                    ...
                    {
                        string cat = GetCategoryForRecord(e.Type);
                        if (!groups.ContainsKey(cat)) groups[cat] = new List<Data.Item>();
                        groups[cat].Add(e);
                    }
                }
                return groups;
            ...
            filterSw.Stop();
            int totalFiltered = grouped.Values.Sum(g => g.Count);
            GD.Print($"[性能] 过滤完成, 耗时={filterSw.ElapsedMilliseconds}ms, 过滤后={totalFiltered}条, 分类数={grouped.Count}");

            // 清空旧数据
            _pendingItems = grouped;
            _tabPopulated.Clear();
            _tabSearchMode.Clear();
            _tabSearchText.Clear();
            ...
            // 关键：为每个分类创建空标签页（Tree），但不填充内容
            foreach (string catName in grouped.Keys)
            {
                GetCategoryTab(catName);
            }
            ...
            if (currentTab >= 0 && currentTab < _tabs.GetTabCount())
            {
                var currentScroll = _tabs.GetChild<ScrollContainer>(currentTab);
                string currentCat = currentScroll.GetMeta("raw_category_name").AsString();
                PopulateTabLazy(currentCat, isSearching, searchText, currentSearchId);
            }
        }

        private const int CHUNK_SIZE = 500;
        private const int FIRST_SCREEN_COUNT = 100;

        private void PopulateTabLazy(string categoryName, bool isSearching, string searchText, int searchId)
        {
            if (_tabPopulated.ContainsKey(categoryName) && _tabPopulated[categoryName])
            {
                return; // 已经填充过
            }
            ...
            _tabSearchMode[categoryName] = isSearching;
            _tabSearchText[categoryName] = searchText;
            _tabPopulated[categoryName] = false; // 标记为正在填充

            PopulateTreeChunkedForCategory(categoryName, isSearching, searchText, searchId);
        }

        private async void PopulateTreeChunkedForCategory(string categoryName, bool isSearching, string searchText, int searchId)
        {
            var populateSw = Stopwatch.StartNew();

            if (!_pendingItems.TryGetValue(categoryName, out var items) || items.Count == 0)
            {
                _tabPopulated[categoryName] = true;
                return;
            }
            ...
            GD.Print($"[性能] 开始填充分类 '{categoryName}', 条目={total}, 首屏={FIRST_SCREEN_COUNT}");

            // 首屏
            int firstScreenEnd = Math.Min(FIRST_SCREEN_COUNT, total);
            for (int i = 0; i < firstScreenEnd; i++)
            {
                AddTreeItemToCategory(items[i], categoryName, isSearching, searchText);
            }
            index = firstScreenEnd;
            chunkCount++;

            // yield 一帧让 Godot 渲染
            await ToSignal(GetTree(), SceneTree.SignalName.ProcessFrame);

            long firstScreenMs = populateSw.ElapsedMilliseconds;
            GD.Print($"[性能] 分类 '{categoryName}' 首屏完成: {firstScreenEnd}条, 耗时={firstScreenMs}ms");

            // 剩余分块
            while (index < total && _populateRunning && _populateSearchId == searchId)
            {
                int end = Math.Min(index + CHUNK_SIZE, total);
                ...
                if (chunkCount % 10 == 0 || index >= total)
                {
                    long elapsed = populateSw.ElapsedMilliseconds;
                    int pct = (int)((float)index / total * 100);
                    GD.Print($"[性能] 分类 '{categoryName}' 进度: {index}/{total} ({pct}%), 已耗时={elapsed}ms");
                }
            ...
            populateSw.Stop();
            GD.Print($"[性能] 分类 '{categoryName}' 填充完成, 总耗时={populateSw.ElapsedMilliseconds}ms");
            _tabPopulated[categoryName] = true;
        ...
        private void AddTreeItemToCategory(Data.Item entry, string categoryName, bool isSearching, string searchText)
        {
            var tree = GetCategoryTab(categoryName);
            if (tree == null) return;
            ...
            {
                displayOriginal = Regex.Replace(displayOriginal, Regex.Escape(searchText), m => m.Value.ToUpper(), RegexOptions.IgnoreCase);
                if (!string.IsNullOrEmpty(displayTranslated) && displayTranslated != "[未翻译]")
                {
                    displayTranslated = Regex.Replace(displayTranslated, Regex.Escape(searchText), m => m.Value.ToUpper(), RegexOptions.IgnoreCase);
                }
            }
            ...
            if (entry.Mod != null && entry.Mod.Type == Data.ModType.StringTable && entry.StringId != 0)
            {
                item.SetCustomColor(2, new Color(1, 0.9f, 0.3f));
            }
            else
            {
                item.ClearCustomColor(2);
            }
        ...
        private Tree GetCategoryTab(string categoryName)
        {
            ...
            tree.Connect("column_title_clicked", Callable.From((int col, int button) => OnColumnTitleClicked(col)));
        ...
        private void SetupEditor(Data.Item entry, TreeItem selected)
        {
            ...
            else
            {
                ...
                _editW.Position = GetWindow().Position + new Vector2I(50, 50);
                _editW.PopupCentered();
            }
            
            _editW.GrabFocus();
            
            CallDeferred(nameof(DeferredSyncRefToEdit));
        }
        
        private void DeferredSyncRefToEdit()
        {
            CallDeferred(MethodName.SyncRefToEdit);
        }
        
        private void SyncRefToEdit() {
            if (!IsInstanceValid(_editW) || !IsInstanceValid(_refW)) return;
        ...
        public override void _Process(double delta)
        {
            ...
            CheckDictionaryCacheExpiry();
        }
        
        private void CheckDictionaryCacheExpiry()
        {
            if (_cachedDictionary != null)
            {
                DateTime currentTime = DateTime.Now;
                if ((currentTime - _lastDictionaryAccessTime).TotalSeconds > 300)
                {
        ...
        private void ProcessBatchImport(string content, bool trimSpaces = false)
        {
            if (_currentProject == null) return;

            var importData = ParseImportContent(content, trimSpaces);
        ...
        //解析导入内容
        private Dictionary<int, string> ParseImportContent(string content, bool trimSpaces = false)
        {
            ...
            foreach (var line in lines)
            {
                ...
                var itemMatch = System.Text.RegularExpressions.Regex.Match(trm, @"^(\d+)\s*[\|｜]\s*(.+)$");
        ...
        private void PerformSaveInternal()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(bakName))
                {
                    ...
                    if (_currentProject.Type == Data.ModType.StringTable)
                    {
                        var alert = new AcceptDialog();
                        alert.Title = L.T("SAVE_STR_TITLE");
                        alert.DialogText = string.Format(L.T("MSG_STR_SAVE_OK"), bakName);
                        AddChild(alert);
                        alert.PopupCentered();
                    }
                    else
        ...
        private void OnBackupRequest()
    {
        ...
        private async void OnBackupRestored()
        {
            if (_currentProject == null) return;
        ...
        private void ExportEntries(List<Data.Item> items)
        {
            ...
            //加载自定义导出提示词
            string exportPrompt = Pos.GetSetting("export_prompt", L.T("PROMPT_PREFIX_GLOSSARY") + L.T("PROMPT_BASE"));
            string prompt = exportPrompt.Replace("{count}", exportedCount.ToString());
            sb.Append("\n\n").Append(prompt);
        ...
        private Dictionary<string, string> LoadCombinedDictionary(string modPath)
        {
            ...
            //只加载默认的其他词典
            string defaultOtherDict = Pos.GetSetting("default_other_dict", "");
            if (!string.IsNullOrEmpty(defaultOtherDict))
            {
                var otherDict = SkyrimModTranslator.Core.Dict.DictStorage.Load("OtherDict", defaultOtherDict);
        ...
        {
            _ = ApplyDictionaryOnSwitchAsync(project);
        }
        
        private void ApplyDictionaryOnSwitch(Data.Mod project)
        {
            _ = ApplyDictionaryOnSwitchAsync(project);
        }

        private async System.Threading.Tasks.Task ApplyDictionaryOnSwitchAsync(Data.Mod project)
        {
            ...
            _isApplyingDictionary = true;
            var sw = Stopwatch.StartNew();
            string modName = Path.GetFileName(project.Path);
            GD.Print($"[性能] 开始应用词典: {modName}, 词条数={project.Items.Count}");

            Dictionary<string, string> glossary;
            if (_cachedDictionary != null && project.Path == _lastModPath)
            {
                glossary = _cachedDictionary;
                GD.Print($"[性能] 使用缓存词典, 条目数={glossary.Count}");
            }
            else
            {
                ...
                var userDict = await System.Threading.Tasks.Task.Run(() => SkyrimModTranslator.Core.Dict.DictStorage.Load("UserDict", ""));
                foreach (var kvp in userDict)
                {
                    glossary[kvp.Key] = kvp.Value;
                }
                ...
                if (!string.IsNullOrEmpty(defaultOtherDict))
                {
                    var otherDict = await System.Threading.Tasks.Task.Run(() => SkyrimModTranslator.Core.Dict.DictStorage.Load("OtherDict", defaultOtherDict));
                    foreach (var kvp in otherDict)
                    {
                        if (!glossary.ContainsKey(kvp.Key))
                        {
                ...
                {
                    string currentModName = Path.GetFileName(project.Path);
                    var modDict = await System.Threading.Tasks.Task.Run(() => SkyrimModTranslator.Core.Dict.DictStorage.Load("ModDict", currentModName));
                    foreach (var kvp in modDict)
                    {
                        if (!glossary.ContainsKey(kvp.Key))
                        {
                ...
                _cachedDictionary = glossary;
                _lastModPath = project.Path;
                _lastDictionaryAccessTime = DateTime.Now;
                loadSw.Stop();
                GD.Print($"[性能] 词典加载完成, 耗时={loadSw.ElapsedMilliseconds}ms, 总条目={glossary.Count}");
            }
            ...
            {
                GD.Print($"[性能] 词典为空, 跳过应用");
                _isApplyingDictionary = false;
                return;
            }
            ...
            for (int i = 0; i < totalItems; i += chunkSize)
            {
                ...
                await System.Threading.Tasks.Task.Run(() =>
                {
                    for (int j = i; j < end; j++)
                    {
                        var entry = project.Items[j];
                        if (string.IsNullOrEmpty(entry.Trans))
                        {
                            string trans = null;
                            
                            if (glossary.TryGetValue(entry.Ori, out trans))
                            {
                                entry.Trans = trans;
                                entry.IsDictApplied = true;
                                System.Threading.Interlocked.Increment(ref appliedCount);
                                continue;
                            }
                            
                            string normalized = entry.Ori.Replace("\r\n", "\n").Replace("\r", "\n");
                            if (normalizedDict.TryGetValue(normalized, out trans))
                            {
                                entry.Trans = trans;
                                entry.IsDictApplied = true;
                                System.Threading.Interlocked.Increment(ref appliedCount);
                            ...
                            if (normalizedDict.TryGetValue(trimmed, out trans))
                            {
                                entry.Trans = trans;
                                entry.IsDictApplied = true;
                                System.Threading.Interlocked.Increment(ref appliedCount);
            ...
            sw.Stop();
            GD.Print($"[性能] 词典应用完成: {modName}, 匹配={appliedCount}/{totalItems}, 总耗时={sw.ElapsedMilliseconds}ms");
            _isApplyingDictionary = false;
        }
        ...
        private void OnTabSelected(long idx)
        {
            ...
            // 懒加载：如果该标签页还没填充过，现在填充
            if (idx < _tabs.GetTabCount())
            {
                var scroll = _tabs.GetChild<ScrollContainer>((int)idx);
                string catName = scroll.GetMeta("raw_category_name").AsString();
                if (!_tabPopulated.ContainsKey(catName) || !_tabPopulated[catName])
                {
                    bool isSearching = _tabSearchMode.ContainsKey(catName) && _tabSearchMode[catName];
                    string searchText = _tabSearchText.ContainsKey(catName) ? _tabSearchText[catName] : "";
                    PopulateTabLazy(catName, isSearching, searchText, _populateSearchId);
                }
            }

            sw.Stop();
            string tabName = idx < _tabs.GetTabCount() ? _tabs.GetTabTitle((int)idx) : "?";
            GD.Print($"[性能] 切换标签页: '{tabName}', 耗时={sw.ElapsedMilliseconds}ms");
        }
        ...
        private async void OnFilesSelected(string[] paths)
        {
            ...
            if (fd != null)
            {
                ...
                foreach (var path in paths) await LoadMod(path);
            }
        }
        ...
        private async void OnColumnTitleClicked(int column)
        {
            var currentScroll = _tabs.GetChild<ScrollContainer>(_tabs.CurrentTab);
            if (currentScroll == null) return;

            string categoryName = currentScroll.GetMeta("raw_category_name").AsString();
            if (!_treeMap.TryGetValue(categoryName, out var tree)) return;

            if (_tabPopulated.TryGetValue(categoryName, out bool isPopulated) && !isPopulated)
            {
                return;
            }

            int col = column;
            int currentSort = _columnSort.ContainsKey(categoryName) ? _columnSort[categoryName] : 0;
            int newSort = (currentSort == 0 || currentSort == 2) ? 1 : 2;
            _columnSort[categoryName] = newSort;

            await SortTreeByColumnAsync(tree, categoryName, col, newSort);
        }

        private async System.Threading.Tasks.Task SortTreeByColumnAsync(Tree tree, string categoryName, int columnIndex, int sortDirection)
        {
            ...
            // 1. 提取数据
            var itemDataList = new List<(string[] texts, Variant metadata)>();
            var child = root.GetFirstChild();
            while (child != null)
            {
                var texts = new string[tree.GetColumns()];
                for (int i = 0; i < tree.GetColumns(); i++)
                {
                    texts[i] = child.GetText(i);
                }
                itemDataList.Add((texts, child.GetMetadata(0)));
                child = child.GetNext();
            }

            // 2. 排序（在后台线程）
            var sortedList = await System.Threading.Tasks.Task.Run(() =>
            {
                itemDataList.Sort((a, b) =>
                {
                    string valA = a.texts[columnIndex];
                    string valB = b.texts[columnIndex];
            ...
            for (int i = 0; i < total; i += chunkSize)
            {
                ...
                for (int j = i; j < end; j++)
                {
                    var data = sortedList[j];
                    var newItem = tree.CreateItem(root);
                    for (int c = 0; c < tree.GetColumns(); c++)
                    {
                        newItem.SetText(c, data.texts[c]);
                    }
                    newItem.SetMetadata(0, data.metadata);

                    string key = data.metadata.AsString();
                    if (_map.TryGetValue(key, out var entry))
                    {
                        if (entry.Mod != null && entry.Mod.Type == Data.ModType.StringTable && entry.StringId != 0)
                        {
                            newItem.SetCustomColor(2, new Color(1, 0.9f, 0.3f));
                        }

                        if (entry.IsDictApplied)
                        {
                            newItem.SetCustomColor(3, new Color(0.4f, 0.6f, 1.0f));
                        }
                        else if (entry.IsTranslated)
                        {
                            newItem.SetCustomColor(3, Colors.SpringGreen);
                        }
                        else
                        {
                            newItem.SetCustomColor(3, Colors.Salmon);
                        }
                    }
                }
            ...
            for (int i = 0; i < tree.GetColumns(); i++)
            {
                string title = i == 0 ? "FormID" : (i == 1 ? L.T("COLUMN_TYPE") : (i == 2 ? L.T("COLUMN_ORIGINAL") : L.T("COLUMN_TRANSLATED")));
                tree.SetColumnTitle(i, i == columnIndex ? title + arrow : title);
            }
        }
        ...
        private void OnBatchImportContentSubmitted(string content, bool trimSpaces)
        {
            ProcessBatchImport(content, trimSpaces);
            _batchImportWin = null;
        }
        ...
        private void OnGSearchPressed()
        {
            if (_projects == null || _projects.Count == 0)
            {
                Feedback.Instance.ShowMessage(L.T("WIN_ALERT_TITLE"), L.T("MSG_NO_MOD_LOADED"));
                return;
            }
            
            var win = new GSearch(_projects);
            win.JumpReq += OnGSearchJump;
            GetTree().Root.AddChild(win);
            win.Show();
        }

        private void OnGSearchJump(string modPath, long itemId)
        {
            JumpToItem(modPath, itemId);
        }

        public void JumpToItem(string modPath, long itemId)
        {
            int idx = _projects.FindIndex(m => m.Path == modPath);
            if (idx < 0) return;
            
            if (_selMod.Selected != idx)
            {
                _selMod.Select(idx);
                OnModSelected(idx);
            }
            
            JumpToTreeItem(itemId);
        }
